...
const fileInput = path.join(`project`, `JSON_output`, `abhishek.json`);
const summaryData = JSON.parse(fs.readFileSync(fileInput, "utf8"));

const senderName = "andre";  // name of user
const recipientInfo = summaryData.summary;  // profile of target recipient

async function generateMessage(method) {
    let prompt = "";

    if (method === "email") {
        prompt = `Draft a friendly email for ${senderName} to reconnect with ${recipientInfo.name}.
        Make the email professional but warm. Include relevant discussion topics based on their background:
        - Education: ${recipientInfo.education.institution} (${recipientInfo.education.degree})
        - Experience: ${recipientInfo.experience.map(exp => `${exp.position} at ${exp.company}`).join(", ")}
        - Contact via email if available: ${recipientInfo.contact.email || "No email provided"}
        ...
        prompt = `Generate a structured JSON object for a LinkedIn message from ${senderName} to reconnect with ${recipientInfo.name}.
        The message should be warm, friendly, and relevant to their background.
        Include relevant discussion topics based on their education (${recipientInfo.education.institution}) 
        and experience (${recipientInfo.experience.map(exp => `${exp.position} at ${exp.company}`).join(", ")}). 
...
function saveFile(method, content) {
    ...
    const filename = recipientInfo.name.split(" ")[0].replace(/[^a-zA-Z0-9_-]/g, "_");
    if (method === "linkedin") {
        try {

            const jsonContent = typeof content === "string" ? JSON.parse(content) : content;
            const filePath = path.join(`project`, `data_output`, `${filename}.json`);
        ...
        const filePath = path.join(`project`, `data_output`, `${filename}.txt`);
